...
if ($mini_user_sort=='') $mini_user_sort = $sortingTopics; /* Sort messages default by last answer (0) desc OR 1 - by last new topics */
...
if ($mini_user_sort==1) { $cols=DB_query(10,0); $sus = 10; }
...
$nTop=1;
if (((isset($poForums) and in_array($forum, $poForums) and $isMod!=1) OR (isset($regUsrForums) and in_array($forum, $regUsrForums) and $mini_user_id==0) OR (isset($roForums) and in_array($forum, $roForums))) and $mini_user_id!=1) {
//$main=preg_replace("/(<form.*<\/form>)/is", '', $main);
$main=preg_replace("/(<form.*<\/form>)/is", 'You must login to post.', $main);
$nTop=0;
$newTopicLink='';
}
