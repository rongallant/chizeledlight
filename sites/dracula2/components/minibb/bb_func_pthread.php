...
$lckt=DB_query(43,0);
if ((sizeof($regUsrForums)>0 and in_array($forum,$regUsrForums) and $mini_user_id==0) or $lckt==1 or $lckt==8) {
$errorMSG=$l_forbidden; $correctErr = "<a href=\"JavaScript:history.back(-1)\">$l_back</a>";
$title = $title.$l_forbidden;
echo load_header(); echo ParseTpl(makeUp('main_warning')); return;
}
else {
if (!$mini_user_usr) $mini_user_usr=$l_anonymous;
...
$postText=textFilter($postText,$post_text_maxlength,$post_word_maxlength,1,$disbbcode,1, $mini_user_id, $absolute_path);
$poster_ip=getIP();
...
if ($mini_user_id==1 or DB_query(79,0)) { 
$query=DB_query(20,0);
...
if (isset($emailadmposts) and $emailadmposts==1 and $mini_user_id!=1) {
sendMail ($admin_email, $sub, $msg, $admin_email, $admin_email);
}

//Insert user into email notifies if allowed
$mini_user_email = DB_query(63,$mini_user_id); $mini_user_email = $mini_user_email['email'];
if ($CheckSendMail and emailCheckBox()!='' and substr(emailCheckBox(),0,8)!='<!--U-->' and $mini_user_email!='') DB_query(81,0);
}
else {
